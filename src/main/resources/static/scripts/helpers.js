...
export function timeToIndex(currentTime, inputTime) {
  ...
  const formattedHelsinkiDateTime = formatter.format(currentDate);
  const helsinkiDate = new Date(formattedHelsinkiDateTime);

  const formattedCurrentDate = helsinkiDate.toISOString().slice(0, 10);
